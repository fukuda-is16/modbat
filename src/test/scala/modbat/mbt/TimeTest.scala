...
class TimeTest extends FlatSpec with Matchers {
  "Time1" should "pass with one transition" in {
    val result = ModbatTestHarness.testMain(Array("-s=1", "-n=1", "--no-redirect-out", "--classpath=build/modbat-test.jar", "-s",  "modbat.test.Time1"), ModbatTestHarness.setTestJar)
  ...
  "Time2" should "pass with three transitions" in {
    val result = ModbatTestHarness.testMain(Array("-s=1", "-n=1", "--no-redirect-out", "--classpath=build/modbat-test.jar", "-s",  "modbat.test.Time2"), ModbatTestHarness.setTestJar)
